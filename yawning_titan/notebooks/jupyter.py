...
def reset_default_jupyter_notebooks(overwrite_existing: bool = True):
    ...
    copying the templates over from the library package data if they do not
    already exist. If a notebook does already exist, but it has been edited,
    passing overwrite_existing=True as a parameter will replace the edited
    notebook with the original.
    ...
    Args:
        overwrite_existing:
            A bool to toggle replacing existing edited notebooks on or off.
    """
