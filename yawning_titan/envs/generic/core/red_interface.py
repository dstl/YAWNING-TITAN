...
class RedInterface(RedActionSet):
    ...
    def __init__(self, network_interface):
        ...
        if self.network_interface.settings.red_spread_action:
            ...
            probabilities_set.append(
                self.network_interface.settings.settings.red_spread_action_likelihood
            )
            action_number += 1
        if self.network_interface.settings.red_random_infection_action:
            self.action_dict[action_number] = self.intrude
            action_set.append(action_number)
            probabilities_set.append(
                self.network_interface.settings.red_random_infection_likelihood
            )
            action_number += 1
        if self.network_interface.settings.red_basic_attack_action:
            self.action_dict[action_number] = self.basic_attack
            action_set.append(action_number)
            probabilities_set.append(self.network_interface.settings.red_basic_attack_likelihood)
            action_number += 1
        if self.network_interface.settings.red_do_nothing_action:
            self.action_dict[action_number] = self.do_nothing
            action_set.append(action_number)
            probabilities_set.append(self.network_interface.settings.red_do_nothing_likelihood)
            action_number += 1
        if self.network_interface.settings.red_move_action:
            self.action_dict[action_number] = self.random_move
            action_set.append(action_number)
            probabilities_set.append(self.network_interface.settings.red_move_action_likelihood)
            action_number += 1
    ...
    def perform_action(self) -> Dict[int, Dict[str, List[Union[bool, str, None]]]]:
        ...
        if self.network_interface.settings.red_naturally_spread:
        ...
        if self.network_interface.settings.red_zero_day_action:
