...
class BlueActionSet:
    ...
    def reduce_node_vulnerability(self, node: str) -> Tuple[str, str]:
        ...
        if new < self.network_interface.game_mode.game_rules.gr_node_vuln_lower:
            new = self.network_interface.game_mode.game_rules.gr_node_vuln_lower
    ...
    def make_safe_node(self, node: str) -> Tuple[str, str]:
        ...
        if self.network_interface.game_mode.blue.blue_make_node_safe_modifies_vuln:
            # Modifies the vulnerability by a set amount (cannot increase it past the limit in the config file)
            change_amount = self.network_interface.game_mode.blue.blue_vuln_change_amount_make_safe
            ...
            if new > self.network_interface.game_mode.game_rules.gr_node_vuln_upper:
                new = self.network_interface.game_mode.game_rules.gr_node_vuln_upper
            elif new > self.network_interface.game_mode.game_rules.gr_node_vuln_lower:
                new = self.network_interface.game_mode.game_rules.gr_node_vuln_lower
            self.network_interface.update_single_node_vulnerability(node, new)

        elif self.network_interface.game_mode.blue.blue_make_safe_random_vuln:
            # Gives the node a new random vulnerability
            upper = self.network_interface.game_mode.game_rules.gr_node_vuln_upper
            lower = self.network_interface.game_mode.game_rules.gr_node_vuln_lower
