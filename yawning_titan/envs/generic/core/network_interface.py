...
class NetworkInterface:
    ...
    def __init__(
        ...
        if config is None:
            config = Config()
            config.create_from_file(default_game_mode_path())
        ...
        if self.settings.network_config.high_value_targets:
            self.high_value_targets = self.settings.network_config.high_value_targets 
        ...
        for i in range(0, self.settings.blue.blue_max_deceptive_nodes):
            name = "d" + str(i)
    ...
    def get_number_unused_deceptive_nodes(self):
        ...
        return self.settings.blue.blue_max_deceptive_nodes - self.current_deceptive_nodes
    ...
    def get_current_observation(self) -> np.array:
        ...
        node_connections = []
        if self.settings.observation_space.obs_node_connections:
        ...
        compromised_state = []
        if self.settings.observation_space.obs_compromised_status:
        ...
        vulnerabilities = []
        if self.settings.observation_space.obs_node_vuln_status:
        ...
        avg_vuln = []
        if self.settings.observation_space.obs_avg_vuln:
        ...
        connectivity = []
        if self.settings.observation_space.obs_graph_connectivity:
            connectivity = [self.connectivity]

        # Gets the attacks that the blue agent detected
        attacking_nodes = []
        attacked_nodes = []
        if self.settings.observation_space.obs_attack_sources or self.settings.observation_space.obs_attack_sources:
            ...
            for i in self.detected_attacks:
                ...
                attacked[i[1]] = 1
            if self.settings.observation_space.obs_attack_sources:
                # attacking nodes
        ...
        entry_nodes = []
        nodes = []
        if self.settings.observation_space.obs_special_nodes:
            ...
            if self.settings.game_rules.gr_loss_hvt:
        ...
        skill = []
        if self.settings.observation_space.obs_red_agent_skill:
    ...
    def generate_vulnerability(self) -> float:
        ...
        """
        return round(random.randint((100 * self.settings.game_rules.gr_node_vuln_lower), (100 * self.settings.game_rules.gr_node_vuln_upper)) / 100, 2)
    ...
    def generate_vulnerabilities(self) -> dict:
        ...
        """
        if self.settings.network_config.vulnerabilities:
            return self.settings.network_config.vulnerabilities
    ...
    def set_entry_nodes(self):
        ...
        if self.settings.network_config.entry_nodes is None:
            if self.settings.game_rules.gr_random_entry_nodes:
                try:
                ...
                if self.settings.game_rules.gr_prefer_edge_nodes:
                    weights = list(map(lambda x: (1 / x) ** 4, weights))
                elif self.settings.game_rules.gr_prefer_central_entry:
                ...
                entry_nodes = choice(
                    all_nodes,
                    self.settings.game_rules.gr_num_entry_nodes,
                    replace=False,
                    p=weights_normal,
                )
                ...
                entry_nodes = []
                for i in range(self.settings.game_rules.gr_num_entry_nodes):
                    entry_nodes.append(nodes[i])

            self.entry_nodes = entry_nodes
        self.entry_nodes = self.settings.network_config.entry_nodes
    ...
    def set_high_value_targets(self):
        ...
        if self.settings.game_rules.gr_number_of_high_value_targets > number_possible_high_value:
            warnings.warn(
        ...
        if self.settings.game_rules.gr_loss_hvt:
            if self.settings.network_config.high_value_targets is None:
                self.possible_high_value_targets = []
                # chooses a random node to be the high value target
                if self.settings.game_rules.gr_loss_hvt_random_placement:
                ...
                if self.settings.game_rules.gr_loss_hvt_furthest_away:
                ...
                self.high_value_targets = random.sample(
                    set(self.possible_high_value_targets), self.settings.game_rules.gr_number_of_high_value_targets
                )
            else:
        ...
        for i, j, k in zip(attacking_nodes, target_nodes, success):
            ...
            if self.current_network_variables[j]["deceptive_node"]:
                if k:
                    ...
                    if (
                            100 * self.settings.blue.blue_chance_to_discover_source_deceptive_succeed
                            > random.randint(0, 99)
                    ):
                    ...
                    if (
                            100 * self.settings.blue.blue_chance_to_discover_source_deceptive_failed
                            > random.randint(0, 99)
                    ):
                        self.detected_attacks.append([i, j])
            else:
                ...
                if k is False:
                    if self.settings.blue.blue_discover_failed_attacks:
                        if (
                                100 * self.settings.blue.blue_chance_to_discover_source_failed
                                > random.randint(0, 99)
                        ):
                    ...
                    if (
                        ...
                        if self.settings.blue.blue_discover_attack_source_if_detected:
                            if (
                                    self.settings.blue.blue_chance_to_discover_source_succeed_known
                                    > random.randint(0, 99)
                            ):
                                self.detected_attacks.append([i, j])
                    else:
                        # If the attack succeeded but blue did not detect it
                        if self.settings.blue.blue_chance_to_discover_source_succeed_unknown:
                            if (
                                    100
                                    * self.settings.blue.blue_chance_to_discover_source_succeed_unknown
                                    > random.randint(0, 99)
                            ):
                                self.detected_attacks.append([i, j])
    ...
    def reset(self):
        ...
        if self.settings.reset.reset_move_entry_nodes:
        ...
        if self.settings.reset.reset_random_vulns:
    ...
    def add_deceptive_node(self, node1: str, node2: str) -> Union[bool, str]:
        ...
        if self.base_graph.has_edge(node1, node2):
            ...
            if self.settings.blue.blue_deceptive_node_make_new:
                self.current_network_variables[node_name] = {
                    "vulnerability_score": new_vulnerability,
                    "true_compromised_status": 0,
                    "blue_view_compromised_status": 0,
    ...
    def __immediate_attempt_view_update(self, node: str):
        ...
        if true_state == 1:
            if (
                    random.randint(0, 99)
                    < self.settings.blue.blue_immediate_detection_chance
                    * 100
    ...
    def scan_node(self, node: str) -> None:
        ...
        if self.current_network_variables[node]["blue_knows_intrusion"]:
            ...
            if true_state == 1:
                if (
                        random.randint(0, 99) < self.settings.blue.blue_scan_detection_chance * 100
