...
class Group(AnyNonZeroGroup):
    """Basic implementation of validation group :class: `~yawning_titan.config.toolbox.groups.validation.AnyNonZeroGroup`."""
...
def nested_group() -> Group:
    """A nested test group with a mixture of :class: `~yawning_titan.config.toolbox.item_types.bool_item.BoolItem`'s and :class: `~yawning_titan.config.toolbox.groups.core.UseValueGroup`'s."""
    test_group = Group()
    return test_group
