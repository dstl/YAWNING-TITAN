...
from yawning_titan.config.environment.new_observation_space_config import ObservationSpace
...
def test_default_observation_space_from_legacy(default_observation_space: ObservationSpace):
    """Create a observation_space instance using the default config file."""
    observation_space = ObservationSpace()

    observation_space.set_from_dict(get_default_config_dict_legacy()["OBSERVATION_SPACE"], legacy=True)
